...
-- Internship analysis query examples
SELECT HeartDisease, COUNT(*) AS records FROM heart_clean GROUP BY HeartDisease;
SELECT AgeCategory, ROUND(100 * AVG(CASE WHEN HeartDisease = 'Yes' THEN 1 ELSE 0 END), 2) AS disease_rate_pct FROM heart_clean GROUP BY AgeCategory ORDER BY AgeCategory;
SELECT Smoking, AlcoholDrinking, COUNT(*) AS records, ROUND(100 * AVG(CASE WHEN HeartDisease = 'Yes' THEN 1 ELSE 0 END), 2) AS disease_rate_pct FROM heart_clean GROUP BY Smoking, AlcoholDrinking;
SELECT Race, COUNT(*) AS records, SUM(CASE WHEN HeartDisease = 'Yes' THEN 1 ELSE 0 END) AS heart_disease_cases FROM heart_clean GROUP BY Race ORDER BY heart_disease_cases DESC;
